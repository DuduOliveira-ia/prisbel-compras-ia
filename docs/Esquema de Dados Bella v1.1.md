# Esquema de Dados da Bella — v1.1 (CONTRATO DE SCHEMA)
...
### CONTRATOS_COMPRAS  ← o coração do "contratado → AF direta"
| Campo | Tipo | Exemplo |
|---|---|---|
...
- FORNECEDORES ganha coluna `fornecedor_id` (FOR-001…) para os FKs de CONTRATOS_COMPRAS.
...
1 aba = 1 tabela, mesmo nome em minúsculas (`OBRAS` → `obras`, `CONTRATOS_COMPRAS` → `contratos_compras`). Export CSV → import.
FKs e constraints entram aí de graça porque os IDs já existem e são consistentes.

## Changelog v1.1 (22/07/2026)

- `CONTRATOS` → **`CONTRATOS_COMPRAS`**: descoberto na implantação que a aba
  CONTRATOS já existe (domínio locações). A tabela de contratos de compras ganhou
  nome próprio — também mais claro semanticamente.
- Abas criadas na planilha em 22/07/2026 via workflow utilitário (efêmero, removido),
  com seed PESSOAS: PS-001 Daniela (COMPRADORA, 5531972249393).
