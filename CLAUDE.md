...
| WF7 Bella Chat | `Im4ijv69Fuk0XxKa` | ATIVO | Chat web AO VIVO: GET `/webhook/bella-chat?t=<BELLA_CHAT_TOKEN>` (página, fonte `painel/bella-chat-v0.8.html`) + POST `/webhook/bella-chat-api` (abas OBRAS/PESSOAS/CONTRATOS_COMPRAS/FATOS/PEDIDOS → Gemini multimodal, transcreve áudio; histórico de conversas na aba CONVERSAS injetado na página via `__BELLA_CONVS__`; HABILIDADES DE AÇÃO, ambas em 2 etapas com trava determinística no Code node — propõe → usuário confirma → Gmail: (a) cotação p/ fornecedores da aba FORNECEDORES; (b) cobrança de pendência ao solicitante do pedido — destinatário = col C REMETENTE da fila PEDIDOS, cópia automática à Daniela (const EMAIL_DANIELA no deploy; teste: oliveirae.ti, trocar junto com WF5 ao oficializar). E-mail SEMPRE da tabela, nunca do LLM; guarda anti-alucinação neutraliza endereço não cadastrado em QUALQUER resposta. Após envio de cotação, marca PEDIDOS!L = EM COTAÇÃO (mesmo efeito do painel; assunto unificado "Cotacao - Pedido N ..." que o WF5 lê — patch 10/08 em WF4/WF5, `scripts/patch_unifica_cotacao.mjs`). Deploy: `node scripts/deploy_bella_chat.mjs` (bounce automático; token no `.env`). NUNCA editar no editor do n8n — o script é a fonte. Bateria de regressão: `node scripts/teste_e2e.mjs` (23 casos; registra 1 pedido real de teste por rodada). AÇÃO registrar_pedido: chat completo → grava na aba PEDIDOS automaticamente (nº sequencial). |
...
Aba CONFIG (10/08): parametrização operacional (chave|valor|descricao) — `cobranca_interna_automatica=NAO` (mecanismo no WF3 ainda não implementado, backlog #6); e-mail externo a fornecedor é SEMPRE manual (política, não switch). Editável no Admin (whitelist). Backlog vivo: `docs/BACKLOG.md`.
...
- `values:batchUpdateByDataFilter` **falha em silêncio** (sem erro e sem escrever) para vários casos neste ambiente — já mordeu na criação da CONFIG e no cabeçalho de PESSOAS. Escrever SEMPRE com `values:batchUpdate` (`data:[{range:'ABA!A1', values}]`) ou `values/ABA!A1:append`; conferir `totalUpdatedCells` na resposta.
- Cota do Sheets API é POR MINUTO por usuário: burst de leituras (reset + pollers + testes juntos) derruba nós Google Sheets com "too many requests". WF3/WF5 têm `retryOnFail:true, maxTries:3, waitBetweenTries:5000` nos nós Sheets/Gmail/HTTP (10/08) — erro em workflow de Gmail Trigger PERDE o e-mail (baseline não repete mensagens antigas), então retry é obrigatório em poller de e-mail.
