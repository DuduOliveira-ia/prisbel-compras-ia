...
| WF7 Bella Chat | `Im4ijv69Fuk0XxKa` | ATIVO | Chat web AO VIVO: GET `/webhook/bella-chat?t=<BELLA_CHAT_TOKEN>` (página, fonte `painel/bella-chat-v0.3.html`) + POST `/webhook/bella-chat-api` (abas OBRAS/PESSOAS/CONTRATOS_COMPRAS/FATOS/PEDIDOS → Gemini multimodal, transcreve áudio). Deploy: `node scripts/deploy_bella_chat.mjs` (bounce automático; token no `.env`). NUNCA editar no editor do n8n — o script é a fonte. |
...
1. **WF5 — Leitura de respostas de fornecedores**: IA lê o e-mail livre do fornecedor, faz o de-para item a item → aba COTACOES → comparativo e "melhor cotação" no painel. (Design já validado com o dono; NADA de formulário para fornecedor.)
